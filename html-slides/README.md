...
编辑时使用 `serve examples/media-gallery --watch --port 8080`；保存 Markdown 或素材后，浏览器自动刷新。构建失败会保留上一次成功的页面并显示错误。默认快捷键是空格/右方向键下一页、左方向键上一页；每套课件在问答阶段确认是否需要自定义，设置写在 `slides.config.json`。
